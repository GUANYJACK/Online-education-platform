...
function LearnHome() {
  ...
  const { data: apiSubjects, isLoading } = useCurriculum(userId);
  const hasApiData = Array.isArray(apiSubjects) && apiSubjects.length > 0;
  const subjects = hasApiData ? apiSubjects : mockSubjects;
