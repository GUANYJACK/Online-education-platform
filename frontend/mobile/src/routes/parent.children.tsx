...
import { Plus, Check, QrCode, Smartphone, X, UserPlus } from "lucide-react";
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold">{t(`child.${c.id}.name`)}</h3>
                <p className="text-xs text-muted-foreground/60">{t(`child.${c.id}.grade`)}</p>
                <p className="mt-0.5 text-[11px] text-muted-foreground/40">{t("pc.lastSync", { t: t(`child.${c.id}.sync`) })}</p>
              </div>
              {active && (
